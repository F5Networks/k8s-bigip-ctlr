...
The |csi_k| allows you to expose `Kubernetes Service`_ outside the cluster as virtual server on a BIG-IP® :term:`device` entirely through the `Kubernetes`_ API.
...
Once installed, it watches for `Kubernetes Service`_ objects and F5 Virtual Server Resources stored as Kubernetes `ConfigMap`_ definitions.
...
- ``kubectl`` (the `Kubernetes`_ CLI) installed, and configured with admin access the cluster.
...
To install the |csi_k|, create a `Kubernetes Deployment`_ for the ``f5-k8s-controller``. The deployment creates a `ReplicaSet <http://kubernetes.io/docs/user-guide/replicasets/>`_, that ensures there is always a `Pod <http://kubernetes.io/docs/user-guide/pods/>`_ that running the ``f5-k8s-controller`` container.
...
#. Create a Kubernetes Secret to store the BIG-IP credentials:
   ...
   #. Verify the secret was created correctly. The values are base64 endoed, and will differ from this example:
...
.. important::
    ...
    * A F5 Virtual Server resource defines a virtual server for one (1) port, for one (1) `Kubernetes Service`_.
...
When there are any changes, |csi_k| will configure the BIG-IP accordingly. For each F5 Virtual Server, the |csi_k| will:
...
The ConfigMap that encodes an F5 resource must have a label: ``f5type``, and 2 properties: ``schema``, and ``data``.
    ...
    * The data property contains the F5 Resource that being encoded.
...
The F5 Virtual Server is an F5 resource. It is a json object that must include:
    ...
    - a frontend property describing the Virtual Server on the BIG-IP, such as the virtualAddress.
...
It is specified either :ref:`Standard <csik_config-vs-frontend-vs>`, or to use an :ref:`iApp <csik_config-vs-frontend-iapp>`.
...
The F5 Virtual Server resource uses these properties:
...
If you want to encode this example Virtual Server resource:

    .. literalinclude:: /static/f5-csi_k/example-vs-resource.json

It should be encoded as a ConfigMap, which looks like this as YAML:
