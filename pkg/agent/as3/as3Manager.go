...
func (am *AS3Manager) getUnifiedDeclaration(cfg *AS3Config) as3Declaration {
	...
	baseAS3ConfigSchema := fmt.Sprintf(baseAS3Config, am.ver, am.build, am.ver)
	...
	if cfg.configmap.Data != "" {
		...
		_ = json.Unmarshal([]byte(baseAS3ConfigSchema), &as3Obj)
	}
...
func (am *AS3Manager) getEmptyAs3Declaration(partition string) as3Declaration {
	...
	baseAS3ConfigEmpty := fmt.Sprintf(baseAS3Config, am.ver, am.build, am.ver)
...
func (am *AS3Manager) IsBigIPAppServicesAvailable() error {
	versionstr, releasestr, err := am.PostManager.GetBigipAS3Version()
	am.ver = versionstr
	am.rel = releasestr
	am.build = am.ver + "-" + am.rel
	if err != nil {
		log.Errorf("[AS3] %v ", err)
		return err
	}
	version := versionstr[:strings.LastIndex(versionstr, ".")]
