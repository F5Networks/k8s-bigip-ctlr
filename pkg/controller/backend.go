...
func (aw *AgentWorker) agentWorker() {
	for rsConfig := range aw.LTM.PostManager.postChan {
		...
		*/
		...
		if len(priorityTenants) > 0 {
			aw.postTenantsDeclaration(decl, rsConfig, priorityTenants)
		}
		// Updating the remaining tenants
		if len(updatedTenants) > 0 {
			aw.postTenantsDeclaration(decl, rsConfig, updatedTenants)
		}
...
func (agent *Agent) postTenantsDeclaration(decl as3Declaration, rsConfig ResourceConfigRequest, tenants []string) {
	...
	//agent.APIHandler.publishConfig(cfg)
