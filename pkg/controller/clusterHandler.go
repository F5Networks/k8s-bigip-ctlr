...
func (ch *ClusterHandler) ResourceStatusUpdater() {
	for rscStatus := range ch.statusUpdate.ResourceStatusUpdateChan {
		...
		rscKey := fmt.Sprintf("%s/%s/%s/%s", rscStatus.ResourceKey.kind, rscStatus.ResourceKey.clusterName,
			rscStatus.ResourceKey.namespace, rscStatus.ResourceKey.name)
		if timestamp, ok := ch.statusUpdate.ResourceStatusUpdateTracker.Load(rscKey); ok {
			if timestamp.(metav1.Time).After(rscStatus.ResourceKey.timestamp.Time) {
				log.Debugf("Resource %v status already updated after time: %v. Skipping status update.",
					rscStatus.ResourceObj, rscStatus.ResourceKey.timestamp)
				continue
			}
		}
		// Update the timestamp
		if !rscStatus.ClearKeyFromCache {
			ch.statusUpdate.ResourceStatusUpdateTracker.Store(rscKey, rscStatus.ResourceKey.timestamp)
		} else {
