...
func (ch *ClusterHandler) ResourceStatusUpdater() {
	for rscStatus := range ch.statusUpdate.ResourceStatusUpdateChan {
		...
		rscKey := fmt.Sprintf("%s/%s/%s", rscStatus.ClusterName, rscStatus.Namespace, rscStatus.ResourceName)
...
func (ch *ClusterHandler) UpdateResourceStatus(rscStatus ResourceStatus) {
	defer func() {
		if r := recover(); r != nil {
			log.Errorf("recovered from panic while updating status for resource %s/%s in cluster %s with err %v",
				rscStatus.Namespace, rscStatus.ResourceName, rscStatus.ClusterName, r)
		}
	}()
	...
	if clusterConfig == nil {
		log.Errorf("Failed while updating status of %s/%s. Error: Unable to get cluster config for cluster:%s",
			rscStatus.Namespace, rscStatus.ResourceName, rscStatus.ClusterName)
		return
	}
	...
	case cisv1.VirtualServerStatus:
		...
		informer := ch.getCRInformerForCluster(rscStatus.ClusterName, rscStatus.Namespace)
		if informer == nil {
		...
		if !rscStatus.ClearKeyFromCache {
			item, exists, err := informer.vsInformer.GetIndexer().GetByKey(rscStatus.Namespace + "/" + rscStatus.ResourceName)
			if err != nil {
			...
			var err error
			vs, err = clusterConfig.kubeCRClient.CisV1().VirtualServers(rscStatus.Namespace).Get(context.Background(), rscStatus.ResourceName, metav1.GetOptions{})
			if err != nil {
	...
	case cisv1.TransportServerStatus:
		...
		// which is usually faster.
		if !rscStatus.ClearKeyFromCache {
			item, exists, err := informer.tsInformer.GetIndexer().GetByKey(rscStatus.Namespace + "/" + rscStatus.ResourceName)
			if err != nil {
			...
			var err error
			ts, err = clusterConfig.kubeCRClient.CisV1().TransportServers(rscStatus.Namespace).Get(context.Background(), rscStatus.ResourceName, metav1.GetOptions{})
			if err != nil {
				if errors.IsNotFound(err) {
					// Resource is not found indicates it's actually deleted, so it eliminates the case of watch label
					// removal from the resource. So, no need to go for status update.
	...
	case cisv1.IngressLinkStatus:
		...
		// which is usually faster.
		if !rscStatus.ClearKeyFromCache {
			item, exists, err := informer.ilInformer.GetIndexer().GetByKey(rscStatus.Namespace + "/" + rscStatus.ResourceName)
			if err != nil {
			...
			var err error
			il, err = clusterConfig.kubeCRClient.CisV1().IngressLinks(rscStatus.Namespace).Get(context.Background(), rscStatus.ResourceName, metav1.GetOptions{})
			if err != nil {
				if errors.IsNotFound(err) {
					// Resource is not found indicates it's actually deleted, so it eliminates the case of watch label
					// removal from the resource. So, no need to go for status update.
	...
	case v1.ServiceStatus:
		...
		informer := ch.getCommonInformerForCluster(rscStatus.ClusterName, rscStatus.Namespace)
		if informer == nil {
			updateErr = fmt.Errorf("failed to get informer")
			break
		}
		...
		// which is usually faster.
		if !rscStatus.ClearKeyFromCache {
			item, exists, err := informer.svcInformer.GetIndexer().GetByKey(rscStatus.Namespace + "/" + rscStatus.ResourceName)
			if err != nil {
			...
			var err error
			svc, err = clusterConfig.kubeClient.CoreV1().Services(rscStatus.Namespace).Get(context.Background(), rscStatus.ResourceName, metav1.GetOptions{})
			if err != nil {
				if errors.IsNotFound(err) {
	...
	case routeapi.RouteStatus:
		resourceType = Route
		informer := ch.getNRInformerForCluster(rscStatus.ClusterName, rscStatus.Namespace)
		if informer == nil {
			updateErr = fmt.Errorf("failed to get informer")
			break
		}
		...
		// which is usually faster.
		if !rscStatus.ClearKeyFromCache {
			item, exists, err := informer.routeInformer.GetIndexer().GetByKey(rscStatus.Namespace + "/" + rscStatus.ResourceName)
			if err != nil {
			...
			var err error
			route, err = clusterConfig.routeClientV1.Routes(rscStatus.Namespace).Get(context.Background(), rscStatus.ResourceName, metav1.GetOptions{})
			if err != nil {
				if errors.IsNotFound(err) {
					// Resource is not found indicates it's actually deleted, so it eliminates the case of watch label
					// removal from the resource. So, no need to go for status update.
	...
	default:
		log.Errorf("unknown resource %s/%s of Cluster:%s received for Status Update", rscStatus.Namespace,
			rscStatus.ResourceName, rscStatus.ClusterName)
		return
	}
	...
	if nil != updateErr {
		log.Errorf("Failed to update the status of %s:%s/%s in Cluster %s. Error: %v. Retry will be attempted.", resourceType,
			rscStatus.Namespace, rscStatus.ResourceName, rscStatus.ClusterName, updateErr)
