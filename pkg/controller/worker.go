...
import (
	...
	"github.com/F5Networks/k8s-bigip-ctlr/v2/pkg/ctlrerrors"
			...
			if (appConfig != l4AppConfig{}) {
				...
				if val, ok := ctlr.resources.processedL4Apps[appConfig]; ok {
					if val.timestamp.Before(&svc.CreationTimestamp) {
						msg := fmt.Sprintf("l4 app '%v' already exists with given ip-address/ipam-label, port or bigip route-domain while processing service '%s/%s' of cluster %s. "+
						...
						return ctlrerrors.ContextualError{Err: errors.New(msg)}, false
					}
				}
			}
		}
		...
		if rKey.kind == VirtualServer || rKey.kind == TransportServer || rKey.kind == Service ||
			...
			case Service:
				if svc, ok := rKey.rsc.(*v1.Service); ok {
					if err, ok := ctlr.shouldProcessServiceTypeLB(svc, rKey.clusterName, false); ok || errors.As(err, &ctlrerrors.ContextualError{}) {
