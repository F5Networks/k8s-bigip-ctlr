...
var _ = Describe("Worker Tests", func() {
	...
	BeforeEach(func() {
		...
		mockCtlr.bigIpConfigMap = make(BigIpConfigMap)
	...
	Describe("IPAM", func() {
		...
		It("Create IPAM Custom Resource", func() {
			err := mockCtlr.ipamHandler.CreateIPAMResource()
		...
		It("Get IPAM Resource", func() {
			...
			ipamCR := mockCtlr.ipamHandler.GetIPAMCR()
			Expect(ipamCR).NotTo(BeNil(), "Failed to GET IPAM")
			mockCtlr.ipamHandler.IPAMCR = mockCtlr.ipamHandler.IPAMCR + "invalid"
			ipamCR = mockCtlr.ipamHandler.GetIPAMCR()
		...
		It("Request IP Address", func() {
			...
			for sp, val := range testSpec {
				...
				ip, status := mockCtlr.ipamHandler.RequestIP("test", host, key, resRef)
				Expect(status).To(Equal(ipmanager.Requested), errHint+"Failed to Request IP")
				...
				ip, status = mockCtlr.ipamHandler.RequestIP("", host, key, resRef)
				Expect(status).To(Equal(ipmanager.InvalidInput), errHint+"Failed to validate invalid input")
				Expect(ip).To(BeEmpty(), errHint+"Failed to validate invalid input")
				newIPAMCR := mockCtlr.ipamHandler.GetIPAMCR()
				Expect(reflect.DeepEqual(ipamCR, newIPAMCR)).To(BeTrue(), errHint+"IPAM CR should not be updated")

				ip, status = mockCtlr.ipamHandler.RequestIP("test", host, key, resRef)
				Expect(status).To(Equal(ipmanager.Requested), errHint+"Wrong status")
				...
				ipamCR, _ = mockCtlr.ipamHandler.IpamCli.Update(ipamCR)
				ip, status = mockCtlr.ipamHandler.RequestIP("test", host, key, resRef)
				Expect(ip).To(Equal("10.10.10.1"), errHint+"Invalid IP")
				Expect(status).To(Equal(ipmanager.Allocated), "Failed to fetch Allocated IP")
				ipamCR = mockCtlr.ipamHandler.GetIPAMCR()
				Expect(len(ipamCR.Spec.HostSpecs)).To(Equal(1), errHint+"Invalid number of Host Specs")
				Expect(ipamCR.Spec.HostSpecs[0].IPAMLabel).To(Equal("test"), errHint+"IPAM Request Failed")
				Expect(ipamCR.Spec.HostSpecs[0].Host).To(Equal(host), errHint+"IPAM Request Failed")
				...
				ip, status = mockCtlr.ipamHandler.RequestIP("dev", host, key, resRef)
				Expect(status).To(Equal(ipmanager.Requested), "Failed to Request IP")
				Expect(ip).To(BeEmpty(), errHint+"Invalid IP")
				ipamCR = mockCtlr.ipamHandler.GetIPAMCR()
				...
				ip, status = mockCtlr.ipamHandler.RequestIP("test", "", "", resRef)
				Expect(status).To(Equal(ipmanager.InvalidInput), errHint+"Failed to validate invalid input")
				Expect(ip).To(BeEmpty(), errHint+"Invalid IP")
				newIPAMCR = mockCtlr.ipamHandler.GetIPAMCR()
				Expect(reflect.DeepEqual(ipamCR, newIPAMCR)).To(BeTrue(), errHint+"IPAM CR should not be updated")
				...
				ip, status = mockCtlr.ipamHandler.RequestIP("old", host, key, resRef)
				Expect(ip).To(Equal(""), errHint+"Invalid IP")
				Expect(status).To(Equal(ipmanager.NotRequested), "Failed to identify Stale status")
			}
		})
		...
		It("Release IP Addresss", func() {
			...
			for sp, val := range testSpec {
				...
				ip := mockCtlr.ipamHandler.ReleaseIP("", host, key, resRef)
				...
				ip = mockCtlr.ipamHandler.ReleaseIP("test", host, key, resRef)
				ipamCR = mockCtlr.ipamHandler.GetIPAMCR()
	...
	Describe("Filtering and Validation", func() {
		...
		Describe("Filter Associated VirtualServers", func() {
			...
			It("Host Group with Multiple Hosts", func() {
				...
				Expect(len(rsMap)).To(Equal(1), "Invalid VS count")
				Expect(len(rsMap["crd_1_2_3_5_80"].MetaData.hosts)).To(Equal(1), "Invalid host count")
				...
				Expect(len(rsMap)).To(Equal(1),
					"Invalid VS count")
				Expect(len(rsMap["crd_1_2_3_5_80"].MetaData.hosts)).
	...
	Describe("Processing Resources", func() {
		It("Processing ServiceTypeLoadBalancer", func() {
			...
			resourceConfig := mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
			Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(0), "Resource Config should be empty")
			...
			resourceConfig = mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
			Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(1), "Invalid Resource Configs")
			...
			rsInt, _ := resourceConfig.ltmConfig.Load(partition)
			rsMap := rsInt.(*PartitionConfig).ResourceMap
			Expect(len(rsMap)).To(Equal(0), "Invalid Resource Configs")
		...
		It("Processing External DNS", func() {
			...
			resourceConfig := mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
			rsInt, _ := resourceConfig.gtmConfig.Load(DEFAULT_GTM_PARTITION)
			gtmConfig, _ := rsInt.(GTMPartitionConfig)
			Expect(len(gtmConfig.WideIPs)).To(Equal(1))
			Expect(len(gtmConfig.WideIPs["test.com"].Pools)).To(Equal(1))
			Expect(len(gtmConfig.WideIPs["test.com"].Pools[0].Members)).To(Equal(0))
			Expect(gtmConfig.WideIPs["test.com"].Pools[0].Ratio).To(Equal(4))
			...
			mockCtlr.resources.updatePartitionResourceMap("default", "SampleVS", rsCfg, bigipConfig)
			mockCtlr.processExternalDNS(newEDNS, false)
			resourceConfig = mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
			rsInt, _ = resourceConfig.gtmConfig.Load(DEFAULT_GTM_PARTITION)
			gtmConfig, _ = rsInt.(GTMPartitionConfig)
			Expect(len(gtmConfig.WideIPs)).To(Equal(1))
			Expect(len(gtmConfig.WideIPs["test.com"].Pools)).To(Equal(1))
			Expect(len(gtmConfig.WideIPs["test.com"].Pools[0].Members)).To(Equal(1))

			mockCtlr.processExternalDNS(newEDNS, true)
			resourceConfig = mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
			rsInt, _ = resourceConfig.gtmConfig.Load(DEFAULT_GTM_PARTITION)
			gtmConfig, _ = rsInt.(GTMPartitionConfig)
			Expect(len(gtmConfig.WideIPs)).To(Equal(0))
		})
		...
		It("Processing IngressLink", func() {
			...
			Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(1), "Invalid LTM Config")
			partition := mockCtlr.getPartitionForBIGIP("")
			rsMap, ok := resourceConfig.ltmConfig.Load(partition)
			Expect(ok).To(BeTrue(), "Invalid LTM Config")
			Expect(len(rsMap.(*PartitionConfig).ResourceMap)).To(Equal(1),
			...
			rsMap, ok = resourceConfig.ltmConfig.Load(partition)
			Expect(ok).To(BeTrue(), "Invalid LTM Config")
			Expect(len(rsMap.(*PartitionConfig).ResourceMap)).To(Equal(0),
				"Invalid Resource Config")
	...
	Describe("Test NodeportLocal", func() {
		...
		Describe("Processing Service of type LB with policy", func() {
			It("Processing ServiceTypeLoadBalancer with Policy", func() {
				...
				_ = mockCtlr.processLBServices(svc1, false)
				resourceConfig := mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
				Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(0),
				...
				rsMap := rsInt.(*PartitionConfig).ResourceMap
				Expect(rsMap[rsname].Virtual.SNAT).To(Equal(DEFAULT_SNAT),
					"Invalid Resource Configs")
				Expect(rsMap[rsname].Virtual.PersistenceProfile).To(Equal(
					plc.Spec.Profiles.PersistenceProfile), "Invalid Resource Configs")
				Expect(rsMap[rsname].Virtual.ProfileL4).To(Equal(
					plc.Spec.Profiles.ProfileL4), "Invalid Resource Configs")
				Expect(len(rsMap[rsname].Virtual.LogProfiles)).To(
				...
				rsMap = rsInt.(*PartitionConfig).ResourceMap
				Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(1), "Invalid Resource Configs")
				Expect(rsMap[rsname].Virtual.SNAT).To(Equal(plc.Spec.SNAT),
					"Invalid Resource Configs")
	...
	Describe("Processing Custom Resources", func() {
		...
		BeforeEach(func() {
			...
			mockCtlr.RequestHandler.PostManagers.PostManagerMap[bigipConfig] = mockPM.PostManager
		...
		Describe("Processing Virtual Server", func() {
			...
			It("Virtual Server with Virtual Address", func() {
				...
				mockCtlr.processResources()
				resourceConfig := mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
				Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(0), "Invalid Virtual Server")
				...
				Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(1), "Virtual Server not Processed")
				...
				mockCtlr.processResources()
				resourceConfig = mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
				Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(0), "Virtual Server not deleted")
				...
				mockCtlr.ipamHandler.IpamCli = nil
				mockCtlr.addVirtualServer(vs)
				mockCtlr.processResources()
				resourceConfig = mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
				Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(0), "Invalid VS")
				vs.Spec.VirtualServerAddress = "10.8.0.1"
				...
				rsInt, _ := resourceConfig.ltmConfig.Load(partition)
				rsMap := rsInt.(*PartitionConfig).ResourceMap
				Expect(*rsMap[rsname].Virtual.HttpMrfRoutingEnabled).To(Equal(true), "HttpMrfRoutingEnabled not enabled on VS")
				Expect(len(rsMap[rsname].Virtual.AdditionalVirtualAddresses)).To(Equal(1))
				Expect(rsMap[rsname].Virtual.AdditionalVirtualAddresses[0]).To(Equal("10.16.0.1"))
				//check irules
				Expect(len(rsMap[rsname].Virtual.IRules)).To(Equal(4), "irules not propely attached")
				//check websocket profile
				Expect(rsMap[rsname].Virtual.ProfileWebSocket).To(Equal("/Common/websocket"))
				for _, rule := range rsMap[rsname].Policies[0].Rules {
				...
				mockCtlr.processResources()
				resourceConfig = mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
				Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(1), "Invalid VS")
			...
			It("Virtual Server with IPAM", func() {
				...
				go mockCtlr.responseHandler(mockCtlr.RequestHandler.PostManagers.PostManagerMap[bigipConfig].respChan)
				...
				newIpamCR, _ = mockCtlr.ipamHandler.IpamCli.Update(newIpamCR)
				...
				_, status := mockCtlr.ipamHandler.RequestIP("test", host, key, resRef)
				Expect(status).To(Equal(ipmanager.Allocated), "Failed to fetch Allocated IP")
				resourceConfig = mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
				Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(1), "VS not Processed")
				rsInt, _ := resourceConfig.ltmConfig.Load("test")
				rsMap := rsInt.(*PartitionConfig).ResourceMap
				Expect(*rsMap["crd_10_10_10_1_443"].Virtual.HttpMrfRoutingEnabled).
				...
				_, status = mockCtlr.ipamHandler.RequestIP("test", "", key, resRef)
				Expect(status).To(Equal(ipmanager.Allocated), "Failed to fetch Allocated IP")
				resourceConfig = mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
				Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(1), "Virtual Server not processed")
				rsInt, _ = resourceConfig.ltmConfig.Load("test")
				...
				time.Sleep(10 * time.Millisecond)
				mockCtlr.RequestHandler.PostManagers.PostManagerMap[bigipConfig].respChan <- &agentCfg
				time.Sleep(10 * time.Millisecond)
				...
				config.reqMeta = mockCtlr.Controller.enqueueReq(bigipResourceConfigRequest, bigipConfig)
				mockCtlr.RequestHandler.PostManagers.PostManagerMap[bigipConfig].respChan <- &agentCfg

				agentCfg.as3Config.failedTenants["test"] = struct{}{}
				mockCtlr.RequestHandler.PostManagers.PostManagerMap[bigipConfig].respChan <- &agentCfg
			...
			It("Processing VS with partition", func() {
				...
				mockCtlr.processResources()
				resourceConfig := mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
				Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(1), "Invalid Partition Count")
				rsInt, _ := resourceConfig.ltmConfig.Load("dev")
				rsMap := rsInt.(*PartitionConfig).ResourceMap
				Expect(len(rsMap)).To(Equal(1), "Invalid VS count")
				...
				mockCtlr.processResources()
				resourceConfig = mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
				Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(2), "Invalid Partition count")
				rsInt, _ = resourceConfig.ltmConfig.Load("dev3")
				rsMap = rsInt.(*PartitionConfig).ResourceMap
				Expect(len(rsMap)).To(Equal(1), "Invalid VS count")
				...
				rsMap = rsInt.(*PartitionConfig).ResourceMap
				Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(1), "Invalid Partition count")
				Expect(len(rsMap)).To(Equal(2), "Invalid VS count")
		...
		Describe("Processing Transport Server", func() {
			...
			It("Transport Server Validation", func() {
				...
				mockCtlr.processResources()
				resourceConfig := mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
				Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(0), "Invalid Transport Server")
				...
				mockCtlr.processResources()
				resourceConfig = mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
				Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(1), "Transport Server not processed")

				rscUpdateMeta := agentConfig{
					id:        0,
					as3Config: as3Config{failedTenants: make(map[string]struct{})},
				}
				mockCtlr.RequestHandler.PostManagers.PostManagerMap[bigipConfig].respChan <- &rscUpdateMeta
				bigipConfigRequest := BigIpResourceConfigRequest{
					ltmConfig:  mockCtlr.resources.getLTMConfigDeepCopy(bigipConfig),
					shareNodes: mockCtlr.shareNodes,
					gtmConfig:  mockCtlr.resources.getGTMConfigCopy(bigipConfig),
				}
				config := ResourceConfigRequest{
					bigIpResourceConfig: bigipConfigRequest,
				}
				config.reqMeta = mockCtlr.Controller.enqueueReq(bigipConfigRequest, bigipConfig)
				config.reqMeta = mockCtlr.Controller.enqueueReq(bigipConfigRequest, bigipConfig)
				rscUpdateMeta.id = 3
				mockCtlr.RequestHandler.PostManagers.PostManagerMap[bigipConfig].respChan <- &rscUpdateMeta
			...
			It("Transport Server with IPAM", func() {
				...
				mockCtlr.processResources()
				resourceConfig = mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
				Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(0), "Transport Server not deleted")
				...
				_, status = mockCtlr.ipamHandler.RequestIP("test", "", key, resRef)
				Expect(status).To(Equal(ipmanager.Allocated), "Failed to fetch Allocated IP")
				mockCtlr.ipamHandler.RemoveUnusedIPAMEntries()
			...
			It("Transport Server with Partition", func() {
				...
				Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(1), "Invalid Partition Count")
				resourceConfig = mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
				rsInt, _ := resourceConfig.ltmConfig.Load(ts.Spec.Partition)
				rsMap := rsInt.(*PartitionConfig).ResourceMap
				Expect(len(rsMap)).To(Equal(1), "Invalid TS Count")
				...
				resourceConfig = mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
				Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(2), "Invalid Partition count")
				rsInt, _ = resourceConfig.ltmConfig.Load("dev2")
				rsMap = rsInt.(*PartitionConfig).ResourceMap
				Expect(len(rsMap)).To(Equal(1), "Invalid TS count")
				...
				mockCtlr.processResources()
				resourceConfig = mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
				Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(3), "Invalid Partition count")
				rsInt, _ = resourceConfig.ltmConfig.Load("dev")
				rsMap = rsInt.(*PartitionConfig).ResourceMap
				Expect(len(rsMap)).To(Equal(1), "Invalid TS count")
				rsInt, _ = resourceConfig.ltmConfig.Load("dev2")
				...
				rsInt, _ = resourceConfig.ltmConfig.Load("dev2")
				rsMap = rsInt.(*PartitionConfig).ResourceMap
				Expect(len(rsMap)).To(Equal(2), "Invalid TS count")
		...
		Describe("Processing EDNS", func() {
			...
			It("EDNS", func() {
				...
				mockCtlr.processResources()
				Expect(LenSyncMap(mockCtlr.resources.bigIpConfigMap[bigipConfig].gtmConfig)).To(Equal(1), "EDNS not processed")

				mockCtlr.deleteEDNS(newEDNS)
				mockCtlr.processResources()
				resourceConfig := mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
				rsInt, _ := resourceConfig.gtmConfig.Load(DEFAULT_GTM_PARTITION)
				gtmConfig := rsInt.(GTMPartitionConfig)
				Expect(len(gtmConfig.WideIPs)).To(Equal(0), "EDNS  not deleted")
			})
		...
		Describe("Processing Ingress Link", func() {
			It("Ingress Link", func() {
				...
				mockCtlr.processResources()
				resourceConfig := mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
				Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(0), "Invalid IngressLink")
				...
				mockCtlr.processResources()
				resRef := ipmanager.ResourceRef{
					Namespace: IngressLink1.Namespace,
					Name:      IngressLink1.Name,
					Kind:      IngressLink,
				}
				_, status = mockCtlr.ipamHandler.RequestIP("test", host, key, resRef)
				resourceConfig = mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
				Expect(status).To(Equal(ipmanager.Allocated), "Failed to fetch Allocated IP")
				Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(1), "IngressLink not processed")
			...
			It("Ingress Link with partition", func() {
				...
				rsInt, _ := resourceConfig.ltmConfig.Load("dev")
				rsMap := rsInt.(*PartitionConfig).ResourceMap
				Expect(len(rsMap)).To(Equal(1), "Invalid IL count")
				...
				mockCtlr.processResources()
				resourceConfig = mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
				Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(1), "Invalid IngressLink")
				rsInt, _ = resourceConfig.ltmConfig.Load("dev")
				rsMap = rsInt.(*PartitionConfig).ResourceMap
				Expect(len(rsMap)).To(Equal(1), "Invalid IL count")
				...
				resourceConfig = mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
				Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(2), "Invalid Partition count")
				rsInt, _ = resourceConfig.ltmConfig.Load("dev1")
				rsMap = rsInt.(*PartitionConfig).ResourceMap
				Expect(len(rsMap)).To(Equal(1), "Invalid IL count")
				...
				rsInt, _ = resourceConfig.ltmConfig.Load("dev")
				rsMap = rsInt.(*PartitionConfig).ResourceMap
				Expect(len(rsMap)).To(Equal(2), "Invalid IL count")
	...
	Describe("Processing Native Resources", func() {
		...
		BeforeEach(func() {
			...
			mockPM = newMockPostManger()
			mockPM.AS3PostManager.AS3Config = cisapiv1.AS3Config{DebugAS3: true}
			mockPM.defaultPartition = "test"
			//					mockPM.AS3PostDelay =
			mockPM.setupBIGIPRESTClient()
			tnt := "test"
...
`
				...
				_ = json.Unmarshal([]byte(newSpec), &newEs)
...
`
			...
			It("Test Readiness Probe", func() {
				...
				mockCtlr.processResources()
				resourceConfig := mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
				Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(1), "Route not processed")
				partition := mockCtlr.getPartitionForBIGIP("")
				rsInt, _ := resourceConfig.ltmConfig.Load(partition)
				rsMap := rsInt.(*PartitionConfig).ResourceMap
				Expect(rsMap["nextgenroutes_443"].Virtual.AutoLastHop).
					To(Equal("default"), "auto last hop not processed")
				Expect(*rsMap["nextgenroutes_443"].Virtual.HttpMrfRoutingEnabled).
					To(Equal(true), "http mrf route not processed")
				Expect(len(rsMap["nextgenroutes_443"].Monitors)).
					To(Equal(1), "readiness-based health monitor not processed")
				Expect(*rsMap["nextgenroutes_80"].Virtual.HttpMrfRoutingEnabled).
					To(Equal(false), "http mrf route not processed")
				Expect(len(rsMap["nextgenroutes_80"].Monitors)).
					To(Equal(1), "readiness-based health monitor not processed")
				Expect(rsMap["nextgenroutes_80"].Monitors[0].Type).
					To(Equal("http"), "readiness-based health monitor not processed")
				Expect(rsMap["nextgenroutes_443"].Monitors[0].Type).
					To(Equal("http"), "readiness-based health monitor not processed")
			...
			It("Test http profile analytics with routes", func() {
				...
				rsInt, _ := resourceConfig.ltmConfig.Load(partition)
				rsMap := rsInt.(*PartitionConfig).ResourceMap
				Expect(rsMap["nextgenroutes_80"].Virtual.AnalyticsProfiles.HTTPAnalyticsProfile).
					To(Equal("/Common/test"), "http profile analytics not processed correctly")
				Expect(rsMap["nextgenroutes_443"].Virtual.AnalyticsProfiles.HTTPAnalyticsProfile).
					To(Equal("/Common/test"), "http profile analytics not processed correctly")
...
`
			...
			It("Process Pass-through Route", func() {
				...
				mockCtlr.processResources()
				resourceConfig := mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
				Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(0), "Invalid Controller Mode")
				...
				mockCtlr.processResources()
				resourceConfig = mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
				Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(0), "Invalid ltm config")
	...
	Describe("Processing VS, TS, IL, SvcLB on pod update", func() {
		...
		It("Virtual Server processing on pod update", func() {
			...
			mockCtlr.processResources()
			resourceConfig = mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
			Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(1), "Invalid Virtual Server")
			mockCtlr.updatePod(pod)
			mockCtlr.processResources()
			resourceConfig = mockCtlr.resources.getBigIpResourceConfig(bigipConfig)
			Expect(LenSyncMap(resourceConfig.ltmConfig)).To(Equal(1), "Invalid Virtual Server")
